...
import '../../../../../core/widgete/toast.dart';
import '../../manger/similar book manger/similar book cubit.dart';
import '../../manger/similar book manger/similar book state.dart';
...
class _BookDetailsBodyState extends State<BookDetailsBody> {
  ...
  void initState() {
    ...
    BlocProvider.of<SimilarBookCubit>(context)
        .getData(category: widget.book.volumeInfo.categories![0]);
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 6),
              BookRating(
                  rates: widget.book.volumeInfo.ratingsCount != null
                      ? widget.book.volumeInfo.ratingsCount.toString()
                      : '0',
                  ratingPercentage: widget.book.volumeInfo.averageRating != null
                      ? widget.book.volumeInfo.averageRating.toString()
                      : '0'),
                ...
                children: [
                  Expanded(
                    child: CustomButton(
                      ...
                      text: Text(
                        widget.book.saleInfo?.retailPrice?.amount == null
                            ? 'Free'
                            : widget.book.saleInfo!.retailPrice!.amount
                                .toString(),
                      ...
                      onPressed: () async {
                        if (widget.book.accessInfo?.pdf?.isAvailable == true) {
                          Uri url = Uri.parse(
                              widget.book.accessInfo?.pdf?.acsTokenLink ?? '');
                  ...
                  Expanded(
                    child: CustomButton(
                      ...
                      onPressed: () async {
                        Uri url = Uri.parse(
                            widget.book.accessInfo?.webReaderLink ?? '');
                        if (await canLaunchUrl(url)) {
                          await launchUrl(
                            url,
                            mode: LaunchMode.externalApplication,
